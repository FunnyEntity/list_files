...
std::string output_list(const std::vector<FileInfo>& files, const Options& opt)
{
	...
	{
		std::string path_str = (opt.relative ? f.rel_path.string() : f.path.string());
		size_t display_width = get_display_width(path_str);
		max_path_width = std::max(max_path_width, display_width);
	}

	// 第二遍：基于最长路径显示宽度计算目标 Tab 位置并输出
	for (const auto& f : files)
	{
		std::string path_str = (opt.relative ? f.rel_path.string() : f.path.string());
		std::string line = path_str;

		// 计算目标 Tab 位置（基于最长路径的显示宽度）
		int target_tab_pos = (max_path_width + 7) / 8 + 1;  // 最长路径后的下一个 Tab 位置

		// 计算当前文件名的显示宽度
		size_t path_width = get_display_width(path_str);
		int current_tab_pos = (path_width + 7) / 8;

		// 计算需要添加的 Tab 数量（至少 1 个）
		int tabs_needed = std::max(1, target_tab_pos - current_tab_pos);
...
void write_output(const std::string& content, const Options& opt, const std::string& target_dir)
{
	if (opt.output.empty())
	{
		std::cout << content;
	...
	{
		std::wstring out_file_w;

		// 如果输出是简单文件名（不包含路径分隔符），将其放置在目标目录旁边
		if (opt.output.find('/') == std::string::npos && opt.output.find('\\') == std::string::npos)
		{
			fs::path target_path(utf8_to_wstring(target_dir));
			fs::path parent_path = target_path.parent_path();
			if (!parent_path.empty())
			{
				out_file_w = (parent_path / utf8_to_wstring(opt.output)).wstring();
			}
			else
			{
				out_file_w = utf8_to_wstring(opt.output);
			}
		}
		else
		{
		...
		if (opt.compress)
		{
			out_file_w += utf8_to_wstring(".gz");
			// TODO: 实现 gzip 压缩
			std::cerr << "Warning: gzip compression not implemented, outputting uncompressed file\n";
		}

		// 使用 _wfopen 正确处理中文路径
		FILE* fp = _wfopen(out_file_w.c_str(), L"wb");
		if (!fp)
		{
			std::cerr << "Error: Cannot create output file - " << wstring_to_utf8(out_file_w) << "\n";
			return;
		}

		// 写入 UTF-8 BOM 以确保正确的编码检测
		...
		std::cout << "Output saved to: " << wstring_to_utf8(out_file_w) << "\n";
	}
}
...
int main(int argc, char** argv)
{
	...
	if (!wargv)
	{
		std::cerr << "Error: Failed to get command line arguments\n";
		return 1;
	}
	...
	utf8_args.reserve(wargc);  // 预先分配空间，防止重新分配导致指针失效
	...
	if (!parse_arguments(wargc, utf8_argv.data(), opt, dir))
	{
		print_usage(utf8_argv[0]);
		return 1;
	}
	...
	{
		case 0:
			output = output_tree(files, dir, opt);
			break;

		case 1:
			output = output_json(files, dir, opt);
			break;

		case 2:
			output = output_list(files, opt);
			break;
	}

	write_output(output, opt, dir);
